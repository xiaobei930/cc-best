#!/bin/bash
# Ralph Loop 便捷启动脚本
# 用法: bash .claude/scripts/ralph.sh [mode] [max-iterations] [feature-description]
#
...
case $MODE in
  ...
  feature)
    if [ -z "$FEATURE_DESC" ]; then
      ...
      echo "用法: bash .claude/scripts/ralph.sh feature 50 \"需求描述\""
      exit 1
    fi
    MAX_ITER=${2:-50}
  ...
  *)
    ...
    echo ""
    echo "用法:"
    echo "  bash .claude/scripts/ralph.sh phase 30"
    echo "  bash .claude/scripts/ralph.sh feature 50 \"实现用户登录功能\""
    exit 1
    ;;
esac
